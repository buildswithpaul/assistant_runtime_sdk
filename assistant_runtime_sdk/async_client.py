...
class AsyncAssistantRuntimeClient(BaseAssistantRuntimeClient):
    ...
    # -------------------------------------------------------------------
    # Platform Prompts admin (Task 14)
    # -------------------------------------------------------------------

    async def list_platform_prompts(
        self,
        page: int = 0,
        page_size: int = 50,
        category: Optional[str] = None,
        ...
        """Admin: list AR Platform Prompt records."""
        endpoint, params = self._prepare_list_platform_prompts(
            page, page_size, category, plan_tier, is_active, search,
        )
        return await self._request_get(
            endpoint, params, api_base=self.marketplace_api_base, timeout=timeout,
        )

    async def get_platform_prompt(
        self, name: str, timeout: Optional[float] = None,
    ) -> Optional[Dict[str, Any]]:
        """Admin: fetch a single AR Platform Prompt by name."""
        endpoint, params = self._prepare_get_platform_prompt(name)
        return await self._request_get(
            endpoint, params, api_base=self.marketplace_api_base, timeout=timeout,
        )
    ...
    async def create_platform_prompt(
        ...
        """Admin: create a new AR Platform Prompt record."""
        endpoint, payload = self._prepare_create_platform_prompt(data)
        return await self._request_post_json(
            endpoint, payload, api_base=self.marketplace_api_base, timeout=timeout,
        )

    async def update_platform_prompt(
        self, name: str, data: Dict[str, Any], timeout: Optional[float] = None,
    ) -> Optional[Dict[str, Any]]:
        """Admin: update an existing AR Platform Prompt record."""
        endpoint, payload = self._prepare_update_platform_prompt(name, data)
        return await self._request_post_json(
            endpoint, payload, api_base=self.marketplace_api_base, timeout=timeout,
        )

    async def delete_platform_prompt(
        self, name: str, timeout: Optional[float] = None,
    ) -> Optional[Dict[str, Any]]:
        """Admin: delete an AR Platform Prompt record."""
        endpoint, payload = self._prepare_delete_platform_prompt(name)
        return await self._request_post_json(
            endpoint, payload, api_base=self.marketplace_api_base, timeout=timeout,
        )

    # -------------------------------------------------------------------
    # Platform Skills admin (Task 14)
    # -------------------------------------------------------------------
    ...
    async def list_platform_skills(
        ...
        """Admin: list AR Platform Skill records."""
        endpoint, params = self._prepare_list_platform_skills(
            page, page_size, category, skill_type, plan_tier, is_active, search,
        )
        return await self._request_get(
            endpoint, params, api_base=self.marketplace_api_base, timeout=timeout,
        )
    ...
    async def get_platform_skill(
        ...
        """Admin: fetch a single AR Platform Skill by name."""
        endpoint, params = self._prepare_get_platform_skill(name)
        return await self._request_get(
            endpoint, params, api_base=self.marketplace_api_base, timeout=timeout,
        )
    ...
    async def create_platform_skill(
        ...
        """Admin: create a new AR Platform Skill record."""
        endpoint, payload = self._prepare_create_platform_skill(data)
        return await self._request_post_json(
            endpoint, payload, api_base=self.marketplace_api_base, timeout=timeout,
        )

    async def update_platform_skill(
        self, name: str, data: Dict[str, Any], timeout: Optional[float] = None,
    ) -> Optional[Dict[str, Any]]:
        """Admin: update an existing AR Platform Skill record."""
        endpoint, payload = self._prepare_update_platform_skill(name, data)
        return await self._request_post_json(
            endpoint, payload, api_base=self.marketplace_api_base, timeout=timeout,
        )

    async def delete_platform_skill(
        self, name: str, timeout: Optional[float] = None,
    ) -> Optional[Dict[str, Any]]:
        """Admin: delete an AR Platform Skill record."""
        endpoint, payload = self._prepare_delete_platform_skill(name)
        return await self._request_post_json(
            endpoint, payload, api_base=self.marketplace_api_base, timeout=timeout,
        )
